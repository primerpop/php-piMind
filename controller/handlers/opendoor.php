				...
				foreach ($pins as $pin =>$labels) {
					foreach ($labels as $label => $state ) {
						...
						if (!isset($this->_door_states["/".$sensor_group_name."/".$pin]) && $current_state == 1) {
							...
							//($handler_name,$pin, $label, $event_code, $event_message,$state) {							
							$message = $this->_controller->generate_handler_event(get_class($this),"/".$sensor_group_name."/".$pin,"Door Closed",self::door_kept_open,"OK","Door Closed.",0);
	...
	public function tick() {
		foreach ($this->_door_states as $pin => $opentime) {
			if (time() > $opentime) {
				...
				$message = $this->_controller->generate_handler_event(get_class($this),$pin,"Door Held Open",self::door_kept_open,"Door Held Open",1);
